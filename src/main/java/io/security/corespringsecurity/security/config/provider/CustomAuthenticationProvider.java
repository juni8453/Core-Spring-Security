...
public class CustomAuthenticationProvider implements AuthenticationProvider {
  ...
  /**
   * CustomAuthenticationProvider 클래스에서 실질적으로 인증을 처리하는 로직 AuthenticationManager 에서 받아온 Username,
   * Credential 만 가지고있는 Authentication 타입 객체를 검증하는 로직이다. ID 는 loadUserByUsername() 에서 검증이 됬으니, PW,
   * 추가 검증 후 Authentication(유저 정보, 권한 정보) AuthenticationManager 로 반환해야한다.
   */
  @Override
  ...
  /**
   * ProviderManager 객체에서 유저 인증을 처리할 수 있는 자식부터 부모까지 적절한 Provider 를 찾는 로직이 있는데, 우리가 만든
   * CustomAuthenticationProvider 객체를 찾아 supports() 메서드로 타고 들어오는 것을 처리하는 메서드 즉, Authentication 객체를
   * 검증해 인자로 받은 Authentication 객체 타입과 일치할 때 CustomAuthenticationProvider 클래스가 인증을 처리하도록 조건을 주는 것.
   */
  @Override
